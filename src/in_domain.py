...
def keep_first_five(values, row_examples):
    ...
    # return ",".join(values.split(",")[:row_examples])
...
def train_model(model, optimizer, lr_scheduler, train_dataloader, device, num_epochs, accumulation_steps, validation_df,
    ...
    for epoch in range(num_epochs):
        ...
        if args.data_name == 'dataset-t2d' or args.data_name == 'dataset-limaye' or args.data_name == 'dataset-wikipedia' or args.data_name == 'dataset-turl':
    ...
    for i in range(num_epochs):
        print(f"epoch: {i}, train_loss_value: {train_loss_all[i]}, validation_accuracy: {validation_acc_all[i]}, test_accuracy: {test_acc_all[i]}")
...
def map_to_closest_class(generated_labels):
    ...
    del generated_embeddings, similarities
...
parser.add_argument('--max-length', type=int, default=2048,
                    help="Maximum sequence length for processing input data.")
...
parser.add_argument('--accumulation_steps', type=int, default=8,
                    help="Number of gradient accumulation steps to effectively increase the batch size.")
...
parser.add_argument("--description_type", nargs="+", default=["llama"], type=str,
                    help="Source of class descriptions., gpt or llama")
...
if args.data_name == 'dataset-sota-small':
    ...
    val_csv_file = os.path.join(data_dir, "sota_validation_schema.csv")
    ...
    train_csv_file = os.path.join(data_dir, "train_t2d_doduo.csv")
    val_csv_file = os.path.join(data_dir, "train_t2d_doduo.csv")
    test_csv_file = os.path.join(data_dir, "test_t2d_doduo.csv")
elif args.data_name == 'dataset-limaye':
    # args.accumulation_steps = 4
    args.num_epochs = 50
    args.only_class_list = True
    train_csv_file = os.path.join(data_dir, "train_t2d_doduo.csv")
    ...
    train_csv_file = os.path.join(data_dir, "train_turl_data.csv")
    val_csv_file = os.path.join(data_dir, "validation_turl_data.csv")
...
test_df = create_table_wise_prompt_column(test_df, text_column, label_column, 'col_idx', init_instruction_pre_table,
                                          init_instruction_post_column, tokenizer, max_length)
validation_df = create_table_wise_prompt_column(validation_df, text_column, label_column, 'col_idx',
                                                init_instruction_pre_table, init_instruction_post_column, tokenizer,
                                                max_length)

class_input_ids = tokenizer(class_list, return_tensors="pt", padding=True, truncation=True)["input_ids"].to(device)
...
train_dataset = TrainScenario2DynamicDataset(tokenizer, init_instruction_pre_table, init_instruction_post_column,
                                             args.data_name, train_csv_file, max_length, row_examples, device, args.description_type,
...
model = train_model(model, optimizer, lr_scheduler, train_dataloader, device, num_epochs, accumulation_steps,validation_df, test_df)
